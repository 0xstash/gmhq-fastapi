...
def generate_competition_matrix(user_company_domain):
    ...
    lookalike_companies = fetch_lookalike_companies_via_companyenrich(
        user_company_domain
    )
    if (
        not lookalike_companies
        or not isinstance(lookalike_companies, list)
        or len(lookalike_companies) == 0
    ):
        return None
    ...
    for lookalike in lookalike_companies:
        ...
        competitor_data += (
            f"Description of Competitor #{competitor_index}: {description}\n\n"
        )
        competitor_index += 1
    ...
    Skew the features and specs of the user's company to be more positive so that the features are complete for {{user_company_domain}} in general but not complete for others. 
    ...
    agent = Agent(tasks=[competition_analysis_task])
    ...
    try:
        new_response = (
            "{" + response.lstrip().rstrip().split("{", 1)[1].rsplit("}", 1)[0] + "}"
        )
