...
def get_contacts_tool():
    ...
    people_vector_store_driver = get_pinecone_vector_store_driver("contacts-dev")
    engine = RagEngine(
    ...
    rag_tool = RagTool(
        description="Information about user's contacts on Linkedin and Gmail",
        rag_engine=engine,
        off_prompt=False
    )
    rag_tool.name = "LinkedinGmailContacts"
    return rag_tool
